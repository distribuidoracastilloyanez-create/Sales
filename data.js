(function() {
    ...
    let _tempClosingsData = []; 
    let _recargasDataCache = []; 
    ...
    // Configuración por defecto (Estilos para ExcelJS)
    ...
    // Formatea cantidad según unidad de venta (Caja, Paquete, Unidad)
    function getDisplayQty(qU, p) {
        ...
        // Si no es entero, mostrar decimales de la unidad mayor
    ...
    async function _prepareClosingMatrix(closingData) {
        ...
        // 1. Obtener Metadatos de Productos (para tener Rubro, Segmento, Marca correctos)
        const productsMeta = new Map();
        
        // Llenar con carga inicial
        cargaInicial.forEach(p => productsMeta.set(p.id, { ...p, initialStock: p.cantidadUnidades || 0 }));
        
        // Intentar llenar con inventario actual para datos más recientes
        try {
        ...
        // 4. Agrupar por Rubro
        const rubrosData = {}; // { RUBRO: [prod1, prod2...] }
        productList.forEach(p => {
            // Solo incluir productos que tengan movimiento (venta/obsequio) o stock inicial
            const hasMov = Object.values(clientRowMap).some(c => (c.products[p.id] || 0) > 0);
            if ((p.initialStock > 0) || hasMov) {
                const r = p.rubro || 'SIN RUBRO';
                if (!rubrosData[r]) rubrosData[r] = [];
                rubrosData[r].push(p);
            }
        });
    ...
    async function showClosingDetail(closingId) {
        ...
        try {
            ...
            // Tabs por Rubro
            Object.keys(data.rubrosData).forEach(r => {
                ...
                // Vista simplificada de productos
                let prods = data.rubrosData[r];
                let prodsHTML = prods.map(p => {
                    let totalQty = 0;
    ...
    async function exportSingleClosingToExcel(closingData) {
        ...
        // --- 1. HOJAS POR RUBRO ---
        Object.keys(data.rubrosData).forEach(rubroName => {
            ...
            data.sortedClients.forEach(cli => {
                ...
                stC.numFmt = '#,##0.00'; stC.border = s.cellData.border;
    ...
    window.dataModule = { 
        ...
        _processSalesDataForModal: _prepareClosingData, 
        getDisplayQty,

(function() {
    ...
    async function renderSortableHierarchy(rubroFiltro = '') {
        ...
        try {
            ...
            allSegments.forEach(seg => {
                ...
                if (rubroFiltro && (!segmentsWithProductsInRubro || !segmentsWithProductsInRubro.has(seg.name))) segCont.classList.add('hidden');
    ...
    function addDragAndDropHandlersHierarchy(container) {
        ...
        container.addEventListener('dragstart', e => {
            ...
            placeholder = document.createElement(draggedType === 'segmento' ? 'div' : 'li');
            placeholder.className = 'border-2 border-dashed border-gray-400 h-8 my-1';
        });
        ...
        container.addEventListener('drop', e => {
            e.preventDefault();
            if (draggedItem && placeholder?.parentNode) placeholder.parentNode.insertBefore(draggedItem, placeholder);
        });
    }
    ...
    async function handleGuardarOrdenJerarquia() {
        ...
        segConts.forEach((segCont, index) => {
            ...
            const newMarcaOrder = Array.from(segCont.querySelectorAll('.marca-item')).map(item => item.dataset.marcaName);
        ...
        try {
            ...
            if (window.adminModule?.propagateCategoryOrderChange) await window.adminModule.propagateCategoryOrderChange('segmentos', orderedSegIds);
            _showModal('Éxito', 'Orden guardado.', showInventarioSubMenu);
        } catch (error) { _showModal('Error', error.message); }
    }

    // --- SECCIÓN ACTUALIZADA: RECARGA DE PRODUCTOS ---
            ...
            <div class="p-4 pt-8">
                <div class="container mx-auto">
                    <div class="bg-white/90 backdrop-blur-sm p-8 rounded-lg shadow-xl">
                        ...
                        <p class="text-sm text-center text-gray-500 mb-4">Actualice el stock y genere un registro de recarga automático.</p>
                        ${getFiltrosHTML('recarga')}
                        <div id="recargaListContainer" class="overflow-x-auto max-h-96 border rounded-lg">
                            <p class="p-4 text-center">Cargando productos...</p>
                        </div>
                        <div class="mt-6 flex flex-col sm:flex-row gap-4">
                            <button id="backToInventarioBtn" class="w-full px-6 py-3 bg-gray-400 text-white rounded-lg">Volver</button>
                            <button id="saveRecargaBtn" class="w-full px-6 py-3 bg-teal-600 text-white font-bold rounded-lg shadow-lg hover:bg-teal-700">Confirmar Recarga</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    async function renderRecargaList() {
            ...
            <table class="min-w-full bg-white text-sm">
                <thead class="bg-gray-100 sticky top-0">
                    <tr>
                        ...
                        <th class="p-2 text-center">Stock Actual</th>
                        <th class="p-2 text-center">Nuevo Stock</th>
                    </tr>
                </thead>
                <tbody>`;
        ...
        productos.forEach(p => {
            ...
            const currentStockUnits = p.cantidadUnidades || 0;
            const currentStockDisplay = Math.floor(currentStockUnits / factor);
            const unitLabel = p.ventaPor?.cj ? 'Cj' : (p.ventaPor?.paq ? 'Paq' : 'Und');

            tableHTML += `
                <tr class="border-b hover:bg-gray-50">
                    <td class="p-2 font-medium">
                        ${p.presentacion}<br><span class="text-xs text-gray-400">${p.marca || 'S/M'}</span>
                    </td>
                    <td class="p-2 text-center text-gray-500">
                        ${currentStockDisplay} ${unitLabel}
                    </td>
                    <td class="p-2 text-center">
                        <div class="flex items-center justify-center gap-1">
                            <input type="number" 
                                data-doc-id="${p.id}" 
                                data-factor="${factor}" 
                                data-old-val="${currentStockUnits}"
                                data-name="${p.presentacion}"
                                value="${currentStockDisplay}" 
                                class="w-20 p-1 border rounded text-center recarga-qty-input focus:ring-2 focus:ring-teal-500">
                            <span class="text-xs font-bold text-gray-400">${unitLabel}</span>
                        </div>
                    </td>
                </tr>`;
        });
    ...
    async function handleGuardarRecarga() {
        const inputs = document.querySelectorAll('.recarga-qty-input');
        const batch = _writeBatch(_db);
        ...
        inputs.forEach(input => {
            ...
            const oldUnits = parseInt(input.dataset.oldVal);
            const newStockDisplay = parseInt(input.value);
            const newUnits = newStockDisplay * factor;

            if (!isNaN(newStockDisplay) && newUnits !== oldUnits) {
                const diffUnits = newUnits - oldUnits;
                const ref = _doc(_db, `artifacts/${_appId}/users/${_userId}/inventario`, input.dataset.docId);
                
                batch.update(ref, { cantidadUnidades: newUnits });
                
                recargaDetalles.push({
                    productoId: input.dataset.docId,
                    presentacion: input.dataset.name,
                    unidadesAnteriores: oldUnits,
                    unidadesNuevas: newUnits,
                    diferenciaUnidades: diffUnits,
                    cantidadVisual: newStockDisplay,
        ...
        _showModal('Confirmar Recarga', `¿Desea procesar la recarga de ${count} productos? Se generará un registro histórico.`, async () => {
            _showModal('Progreso', 'Procesando recarga y guardando registro...');
            try {
                // 1. Guardar el registro de la recarga en Firebase
                ...
                // 2. Ejecutar la actualización masiva del inventario
                await batch.commit();

                _showModal('Éxito', `Recarga procesada correctamente. Se actualizaron ${count} productos.`);
                renderRecargaList();
            } catch (error) {
                console.error("Error en recarga:", error);
                _showModal('Error', 'No se pudo completar la recarga: ' + error.message);
            }
        });
    }
    ...
    function validateData(data) {
        if (!data.rubro || !data.segmento || !data.marca || !data.presentacion) return "Todos los campos son obligatorios.";
        if (data.precioPorUnidad <= 0) return "El precio debe ser mayor a 0.";
        return null;
    }
    ...
    function getFiltrosHTML(prefix) {
        return `
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 p-4 bg-gray-50 rounded">
                <input type="text" id="${prefix}-search-input" placeholder="Buscar producto..." class="md:col-span-4 p-2 border rounded">
